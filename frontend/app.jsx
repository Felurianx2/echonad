...
export default function EchoNad() {
  ...
  const grid = useMemo(() => {
    ...
    for (let r = 0; r <= RINGS; r++)
      els.push(<circle key={`r${r}`} cx={CX} cy={CY} r={MIN_R + r * ringW} fill="none" stroke="rgba(131,110,249,0.09)" strokeWidth="0.5" strokeDasharray="3,5" />);
    for (let s = 0; s < SECTORS; s++) {
      const a = s * SECTOR_ANGLE;
      const i = polarToCart(CX, CY, MIN_R, a), o = polarToCart(CX, CY, MAX_R, a);
      els.push(<line key={`l${s}`} x1={i.x} y1={i.y} x2={o.x} y2={o.y} stroke="rgba(131,110,249,0.06)" strokeWidth="0.5" />);
    }
    return els;
  }, []);
        ...
        els.push(
          <g key={k}>
            <path d={d} fill={fill}
              ...
              onMouseEnter={e => { if (!bet && !live) e.target.setAttribute("fill", "rgba(131,110,249,0.08)"); }}
              onMouseLeave={e => { if (!bet && !live) e.target.setAttribute("fill", "transparent"); }} />
            {bet && (
              <>
                <rect x={lp.x - 16} y={lp.y - 7} width="32" height="14" rx="3" fill={live ? MONAD_GREEN : MONAD_PURPLE} opacity="0.85" />
      ...
      <div style={{ flex: 1, display: "flex" }}>
        ...
        <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", position: "relative" }}>
          <svg viewBox="0 0 500 500" style={{ width: "min(500px, 68vh)", height: "min(500px, 68vh)" }}>
            <text x={CX} y={16} textAnchor="middle" fill={MONAD_GREEN} fontSize="9" fontWeight="bold" fontFamily="monospace" opacity="0.5">BULLISH</text>
            <text x={CX} y={492} textAnchor="middle" fill={MONAD_RED} fontSize="9" fontWeight="bold" fontFamily="monospace" opacity="0.5">BEARISH</text>
            {grid}
            {cells}
            {sweepEl}
            {/* PING */}
            ...
            {/* CENTER */}
            <circle cx={CX} cy={CY} r={MIN_R} fill={MONAD_DEEP} stroke="rgba(131,110,249,0.25)" strokeWidth="0.8" />
            <text x={CX} y={CY - 10} textAnchor="middle" fill={MONAD_GOLD} fontSize="12" fontWeight="700" fontFamily="monospace">${curPrice.toFixed(4)}</text>
            <text x={CX} y={CY + 4} textAnchor="middle" fill={delta >= 0 ? MONAD_GREEN : MONAD_RED} fontSize="9" fontFamily="monospace">{delta >= 0 ? "+" : ""}{(delta * 10000).toFixed(1)}bp</text>
            <text x={CX} y={CY + 17} textAnchor="middle" fill="rgba(131,110,249,0.3)" fontSize="7" fontFamily="monospace">MON/USD</text>
            {/* SECTOR LABELS */}
            {SECTOR_META.map((sl, s) => {
              const a = s * SECTOR_ANGLE + SECTOR_ANGLE / 2;
              const p = polarToCart(CX, CY, MAX_R + 16, a);
              return (<g key={s}><text x={p.x} y={p.y - 3} textAnchor="middle" dominantBaseline="middle" fill={s === activeSector ? (sl.up ? MONAD_GREEN : MONAD_RED) : "rgba(131,110,249,0.2)"} fontSize="7" fontWeight={s === activeSector ? "bold" : "normal"} fontFamily="monospace">{sl.shortDir}</text><text x={p.x} y={p.y + 5} textAnchor="middle" dominantBaseline="middle" fill={s === activeSector ? (sl.up ? MONAD_GREEN : MONAD_RED) : "rgba(131,110,249,0.15)"} fontSize="5.5" fontFamily="monospace">{sl.label}</text></g>);
            })}
            {/* RING LABELS */}
            {RING_LABELS.map((l, i) => {
              const p = polarToCart(CX, CY, MIN_R + i * ringW + ringW / 2, 2);
              return <text key={i} x={p.x + 3} y={p.y} fill={i === activeRing ? MONAD_GOLD : "rgba(131,110,249,0.2)"} fontSize="8" fontFamily="monospace" fontWeight={i === activeRing ? "bold" : "normal"} dominantBaseline="middle">{l}</text>;
            })}
            {/* FLOATS */}
            {floats.map(f => <text key={f.id} x={f.x} y={f.y} textAnchor="middle" fill={f.color} fontSize="12" fontWeight="bold" fontFamily="monospace" opacity={f.life}>{f.text}</text>)}
          </svg>
